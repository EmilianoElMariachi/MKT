    ...
    public static class KeyCheck
    {
        ...
        public static List<byte[]> GetPkeyConfigWindows(string product = "")
        {
            ...
            switch (product)
            {
                ...
                case OSVersion.Win11:
                    return new List<byte[]>
                            {
                                    // Cobalt
                                    Resources.pkeyconfig_win11_22000,
                                    // Ferrum
                                    Resources.pkeyconfig_win10_20348,
                                    Resources.pkeyconfig_win10csvlk_20348,
                                    // Vibranium
                                    // Titanium, 19H1
                                    Resources.pkeyconfig_win10_19041,
                                    // Redstone 5
        ...
        public static string CheckKey(string key, byte[] pkeyconfig)
        {
            try
            {
                ...
                if (File.Exists(CommonUtilities.EscapePath(dllPath)) == false)
                {
                    byte[] Pid_Gen;
                    if (!Environment.Is64BitProcess)
                    {
                        Pid_Gen = Resources.pidgenx32;
                    }
                    else
                    {
                        Pid_Gen = Resources.pidgenx64;
                    }
                    using (IArchive aPidgenx = ArchiveFactory.Open(new MemoryStream(Pid_Gen)))
                    {
                        IReader reader = aPidgenx.ExtractAllEntries();
                        while (reader.MoveToNextEntry())
                        {
                            reader.WriteEntryToDirectory(CommonUtilities.EscapePath(Environment.GetEnvironmentVariable("temp")));
                        }
                    }
                }
                ...
                using (IArchive aPKey = ArchiveFactory.Open(new MemoryStream(pkeyconfig)))
                {
                    foreach (IArchiveEntry entry in aPKey.Entries)
                    {
                        pKeyPath = Environment.GetEnvironmentVariable("temp") + Path.DirectorySeparatorChar + Path.GetFileName(entry.Key);
                        if (File.Exists(CommonUtilities.EscapePath(pKeyPath)) == false)
                        {
                            entry.WriteToDirectory(CommonUtilities.EscapePath(Environment.GetEnvironmentVariable("temp")));
                        }
                    }
                }
                ...
                using (StringWriter output = new StringWriter())
                {
                    switch (retID)
                    {
                        ...
                        case -1979645695:
                            ...
                            break;
                        case -1979645951:
                            output.WriteLine("Valid Key but can't be verified");
                            break;
                        case -2147024894:
                            output.WriteLine("pkeyconfig file is not found");
                            break;
                        case -2147024893:
                            output.WriteLine("pkeyconfig path is not found");
                            break;
                        default:
