...
namespace KMSServerServiceInstaller
{
    ...
    public static class KMSServerServiceInstaller
    {
        ...
        private static void MakeKMSServerServiceRegistration()
        {
            ...
            using (RegistryKey registry = Registry.LocalMachine.CreateSubKey("SYSTEM\\CurrentControlSet\\Services\\" + KMSServerServiceName + "\\Parameters"))
            {
                if (registry != null)
                {
                    // Set KMS PID
            ...
            using (StringWriter output = new StringWriter())
            {
                ...
                try
                {
                    try
                    {
                        ...
                        {
                            Services.WinDefendExclusion("Add", CommonUtilities.EscapePath(_kmsServerServiceInstallPath + Path.DirectorySeparatorChar + KMSServerServiceFileName));
                        }
                    ...
                    catch (Exception)
                    {
                        ...
                        if (OSVersion.GetWindowsNumber() >= 6.3)
                        {
                            Services.WinDefendExclusion("Remove", CommonUtilities.EscapePath(_kmsServerServiceInstallPath + Path.DirectorySeparatorChar + KMSServerServiceFileName));
                        }
