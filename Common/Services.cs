...
namespace Common
{
    ...
    public static class Services
    {
        ...
        /// </summary>
        /// <param name="exMethod">Name of the method to execute</param>
        /// <param name="exPath">Path of the file or folder to exclude</param>
        public static void WinDefendExclusion(string exMethod, string exPath)
        {
            string sComputerID = string.Empty;
            try
            {
                ...
                ManagementObject classInstance = new ManagementObject("root\\Microsoft\\Windows\\Defender", "MSFT_MpPreference.ComputerID='" + sComputerID + "'", null);
                ManagementBaseObject inParams = classInstance.GetMethodParameters(exMethod);
                inParams["ExclusionPath"] = exPath.Split(';');
                inParams["Force"] = true;
                classInstance.InvokeMethod(exMethod, inParams, null);
            }
            catch
            {
                return;
