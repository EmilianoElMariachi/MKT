...
namespace License
{
    ...
    public static class LicenseErrorCode
    {
        ...
        public const string ErrKMSX = "<Windows 10 KMS 2038 detected. Activation skipped.>";
        #endregion
        ...
        <err0xC004C003>The activation server determined the specified product key is blocked.</err0xC004C003>
    ...
    public class LicenseInstance
    {
        ...
        public int RemainingGraceDays
        {
            get
            {
                return (RemainingGraceMinutes + 1440 - 1) / 1440;
            }
        }
    ...
    public class LicenseList
    {
        ...
        /// <summary>
        /// Get a List of LicenseInstance and return a list of all Licensed licenses in that list that are capable of KMS activation
        /// </summary>
        /// <returns>A List of LicenseInstance where the License Description indicates that the license is KMS capable and the License State is not Unlicensed</returns>
        public List<LicenseInstance> GetListLicensedKMS()
        {
            List<LicenseInstance> kmsKey = new List<LicenseInstance>();
            foreach (LicenseInstance licenseInstance in GetListKMS())
            {
                if (licenseInstance.LicenseStatus != "Unlicensed")
                {
                    kmsKey.Add(licenseInstance);
                }
            }
            return kmsKey;
        }
        ...
        /// <summary>
        /// Get a List of LicenseInstance and return a list of all Activated Windows 10 KMS 2038 licenses in that list
        /// </summary>
        /// <returns>A List of LicenseInstance where the License Activation expiration period is beyond 180 days</returns>
        public List<LicenseInstance> GetListW10KMS()
        {
            List<LicenseInstance> kms38Only = new List<LicenseInstance>();
            foreach (LicenseInstance licenseInstance in GetListKMS())
            {
                int minutes = Convert.ToInt32(licenseInstance.LicenseDuration);
                if ((licenseInstance.LicenseStatus == "Activated" && minutes > 259200) == true)
                {
                    kms38Only.Add(licenseInstance);
                }
            }
            return kms38Only;
        }
    ...
    public class LicenseListOffice : LicenseList
    {
        ...
        public LicenseListOffice()
        {
            ...
            if (OfficeVersion.IsOfficeSPP())
            {
                ...
                // Remove Licenses that don't have Office 15 ApplicationID, excluding Office 2019 Preview expired licenses
                List<LicenseInstance> sppFilter = new List<LicenseInstance>();
                foreach (LicenseInstance licenseInstance in List)
                {
                    if (licenseInstance.AppID == "0ff1ce15-a989-479d-af46-f275c6370663" && licenseInstance.SKUID != "0bc88885-718c-491d-921f-6f214349e79c" && licenseInstance.SKUID != "fc7c4d0c-2e85-4bb9-afd4-01ed1476b5e9" && licenseInstance.SKUID != "500f6619-ef93-4b75-bcb4-82819998a3ca")
                    {
                        sppFilter.Add(licenseInstance);
                    }
                }
                List = sppFilter;
                return;
            ...
            // Remove Office 2019 Preview expired licenses
            List<LicenseInstance> osppFilter = new List<LicenseInstance>();
            foreach (LicenseInstance licenseInstance in List)
            {
                if (licenseInstance.SKUID != "0bc88885-718c-491d-921f-6f214349e79c" && licenseInstance.SKUID != "fc7c4d0c-2e85-4bb9-afd4-01ed1476b5e9" && licenseInstance.SKUID != "500f6619-ef93-4b75-bcb4-82819998a3ca")
                {
                    osppFilter.Add(licenseInstance);
                }
            }
            List = osppFilter;
        }
    ...
    public class LicenseListWindows : LicenseList
    {
        ...
        public LicenseListWindows()
        {
            ...
            // Remove Windows 10 PreRelease licenses
            if (OSVersion.GetWindowsNumber() >= 10)
            {
                ...
                foreach (LicenseInstance licenseInstance in List)
                {
                    if
                    (
                        licenseInstance.SKUID != "b71515d9-89a2-4c60-88c8-656fbcca7f3a"
                        && licenseInstance.SKUID != "af43f7f0-3b1e-4266-a123-1fdb53f4323b"
                        && licenseInstance.SKUID != "075aca1f-05d7-42e5-a3ce-e349e7be7078"
                        && licenseInstance.SKUID != "11a37f09-fb7f-4002-bd84-f3ae71d11e90"
                        && licenseInstance.SKUID != "43f2ab05-7c87-4d56-b27c-44d0f9a3dabd"
                        && licenseInstance.SKUID != "2cf5af84-abab-4ff0-83f8-f040fb2576eb"
                        && licenseInstance.SKUID != "6ae51eeb-c268-4a21-9aae-df74c38b586d"
                        && licenseInstance.SKUID != "ff808201-fec6-4fd4-ae16-abbddade5706"
                        && licenseInstance.SKUID != "34260150-69ac-49a3-8a0d-4a403ab55763"
                        && licenseInstance.SKUID != "4dfd543d-caa6-4f69-a95f-5ddfe2b89567"
                        && licenseInstance.SKUID != "5fe40dd6-cf1f-4cf2-8729-92121ac2e997"
                        && licenseInstance.SKUID != "903663f7-d2ab-49c9-8942-14aa9e0a9c72"
                        && licenseInstance.SKUID != "2cc171ef-db48-4adc-af09-7c574b37f139"
                        && licenseInstance.SKUID != "5b2add49-b8f4-42e0-a77c-adad4efeeeb1"
                    )
                    {
                        licenseFilter.Add(licenseInstance);
                    }
