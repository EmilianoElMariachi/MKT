...
namespace KMSEmulator.KMS
{
    class KMSPIDGenerator
    {
        ...
        private readonly Guid _kmsCountedIDOffice2021           = new Guid("86d50b16-4808-41af-b83b-b338274318b2"); // Office 2021
        ...
        private readonly Guid _kmsCountedIDWindows10UnknownR    = new Guid("cbdcb1ba-e093-4c81-a463-10775291fdf9"); // Windows 10 (Retail) Unknown
        private readonly Guid _kmsCountedIDWindows10UnknownV    = new Guid("d27cd636-1962-44e9-8b4f-27b6c23efb85"); // Windows 10 (Volume) Unknown
        ...
        private readonly Guid _kmsCountedIDWindowsServer2022    = new Guid("b74263e4-0f92-46c6-bcf8-c11d5efe2959"); // Windows Server 2022
        ...
        private const int HostServer2022 = 6;
        ...
        private const int PkeyConfigWindowsServer2019VR = 7;
        private const int PkeyConfigWindowsServer2022 = 8;
        private const int PkeyConfigOffice14 = 9;
        private const int PkeyConfigOffice15 = 10;
        private const int PkeyConfigOffice16 = 11;
        private const int PkeyConfigOffice19 = 12;
        private const int PkeyConfigOffice21 = 13;
        ...
        public string CreateKMSPID(KMSRequest request)
        {
            ...
            {
                new HostOS { Type = 55041, OSBuild = 6002, MinimumDay = 146, MinimumYear = 2009},  // Windows Server 2008:    2009/05/26
                new HostOS { Type = 55041, OSBuild = 7601, MinimumDay =  53, MinimumYear = 2011},  // Windows Server 2008 R2: 2011/02/22
                new HostOS { Type = 5426, OSBuild =  9200, MinimumDay = 248, MinimumYear = 2012},  // Windows Server 2012:    2012/09/04
                new HostOS { Type = 6401, OSBuild =  9600, MinimumDay = 290, MinimumYear = 2013},  // Windows Server 2012 R2: 2013/10/17
                new HostOS { Type = 3612, OSBuild = 14393, MinimumDay = 286, MinimumYear = 2016},  // Windows Server 2016:    2016/10/12
                new HostOS { Type = 3612, OSBuild = 17763, MinimumDay = 275, MinimumYear = 2018},  // Windows Server 2019:    2018/10/02
                new HostOS { Type = 3612, OSBuild = 20348, MinimumDay = 230, MinimumYear = 2021}   // Windows Server 2022:    2021/08/18
            };
            ...
            List<PkeyConfig> pkeyConfigList = new List<PkeyConfig>
            {
                ...
                new PkeyConfig { GroupID = 206, PIDRangeMin = 2835000, PIDRangeMax = 2854999 },     // Windows Server 2019 VL Retail
                new PkeyConfig { GroupID = 4573, PIDRangeMin = 30000, PIDRangeMax = 20029999 },     // Windows Server 2022
                ...
                new PkeyConfig { GroupID = 206, PIDRangeMin = 571000000, PIDRangeMax = 590999999 }  // Office 2021
            };

            // Product Specific Detection
            int osTypeIndex = HostServer2019;
            int keyConfigIndex = PkeyConfigWindowsServer2019;
            if (request.ApplicationId == _appIDOffice2010 || request.ApplicationId == _appIDOffice2013)
            {
                if (request.KmsCountedId == _kmsCountedIDOffice2010)
                {
                    keyConfigIndex = PkeyConfigOffice14;
                    osTypeIndex = Random.Next(HostServer2012, HostServer2012R2 + 1);
                }
                ...
                else if (request.KmsCountedId == _kmsCountedIDOffice2016)
                {
                    ...
                    osTypeIndex = Random.Next(HostServer2012, HostServer2022 + 1);
                }
                else if (request.KmsCountedId == _kmsCountedIDOffice2019)
                {
                    keyConfigIndex = PkeyConfigOffice19;
                    osTypeIndex = Random.Next(HostServer2012R2, HostServer2022 + 1);
                }
                else if (request.KmsCountedId == _kmsCountedIDOffice2021)
                {
                    keyConfigIndex = PkeyConfigOffice21;
                    osTypeIndex = Random.Next(HostServer2016, HostServer2022 + 1);
                }
                else
                {
                    keyConfigIndex = PkeyConfigOffice16;
            ...
            else if (request.ApplicationId == _appIDWindows)
            {
                ...
                {
                    keyConfigIndex = Random.Next(PkeyConfigWindowsServer2012, PkeyConfigWindowsServer2022 + 1);
                }
                ...
                else if (request.KmsCountedId == _kmsCountedIDWindows8Volume || request.KmsCountedId == _kmsCountedIDWindowsServer2012)
                {
                    keyConfigIndex = Random.Next(PkeyConfigWindowsServer2012, PkeyConfigWindowsServer2022 + 1);
                }
                else if (request.KmsCountedId == _kmsCountedIDWindows81Volume || request.KmsCountedId == _kmsCountedIDWindowsServer2012R2)
                {
                    keyConfigIndex = Random.Next(PkeyConfigWindowsServer2012R2, PkeyConfigWindowsServer2022 + 1);
                }
                else if (request.KmsCountedId == _kmsCountedIDWindows10Volume || request.KmsCountedId == _kmsCountedIDWindows10UnknownV || request.KmsCountedId == _kmsCountedIDWindowsServer2016)
                {
                    keyConfigIndex = Random.Next(PkeyConfigWindowsServer2016, PkeyConfigWindowsServer2022 + 1);
                }
                else if (request.KmsCountedId == _kmsCountedIDWindows10LTSB2016)
                {
                    keyConfigIndex = Random.Next(PkeyConfigWindowsServer2016, PkeyConfigWindowsServer2019 + 1);
                }
                else if (request.KmsCountedId == _kmsCountedIDWindowsServer2019)
                {
                    keyConfigIndex = Random.Next(PkeyConfigWindowsServer2019, PkeyConfigWindowsServer2022 + 1);
                }
                else if (request.KmsCountedId == _kmsCountedIDWindows10LTSC2019)
                {
                    keyConfigIndex = PkeyConfigWindowsServer2019;
                }
                else if (request.KmsCountedId == _kmsCountedIDWindowsServer2022)
                {
                    keyConfigIndex = PkeyConfigWindowsServer2022;
                }
                else if (request.KmsCountedId == _kmsCountedIDWindows8Retail || request.KmsCountedId == _kmsCountedIDWindows81Retail || request.KmsCountedId == _kmsCountedIDWindows10Retail || request.KmsCountedId == _kmsCountedIDWindows10UnknownR)
                {
                    keyConfigIndex = PkeyConfigWindowsServer2019VR;
                }
                ...
                if (keyConfigIndex == PkeyConfigWindowsServer2022)
                {
                    osTypeIndex = Random.Next(HostServer2016, HostServer2022 + 1);
                }
                else if (keyConfigIndex == PkeyConfigWindowsServer2019 || keyConfigIndex == PkeyConfigWindowsServer2019VR)
                {
                    osTypeIndex = Random.Next(HostServer2012R2, HostServer2019 + 1);
                }
                else if (keyConfigIndex == PkeyConfigWindowsServer2016)
                {
                    osTypeIndex = Random.Next(HostServer2012, HostServer2016 + 1);
                }
                else if (keyConfigIndex == PkeyConfigWindowsServer2012R2)
                {
                    osTypeIndex = Random.Next(HostServer2012, HostServer2012R2 + 1);
                }
                else if (keyConfigIndex == PkeyConfigWindowsServer2012)
                {
                    osTypeIndex = HostServer2012;
                }
                else if (keyConfigIndex == PkeyConfigWindows10ChinaGov)
                {
