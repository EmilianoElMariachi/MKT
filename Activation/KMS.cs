...
namespace Activation
{
    ...
    public static class KMSConnection
    {
        ...
        public static void RemoveKMSHostOffice(LicenseList licenses)
        {
            ...
            else
            {
                ...
                RemoveKMSHostRegistry(licenses, @"SOFTWARE\Microsoft\OfficeSoftwareProtectionPlatform");
            }

            // DiscoveredKeyManagementServiceIpAddress for Windows 8.1 and 10
            if (OfficeVersion.IsOfficeSPP() && OSVersion.GetWindowsNumber() >= 6.3)
            {
                using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.Users, RegistryView.Registry64).OpenSubKey(@"S-1-5-20\Software\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform", true))
                {
                    if (registry != null)
                    {
                        registry.DeleteSubKeyTree("0ff1ce15-a989-479d-af46-f275c6370663", false);
                    }
                }
            }
        }
        ...
        public static void RemoveKMSHostWindows(LicenseList licenses)
        {
            ...
            // DiscoveredKeyManagementServiceIpAddress for Windows 8.1 and 10
            if (OSVersion.GetWindowsNumber() >= 6.3)
            {
                using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.Users, RegistryView.Registry64).OpenSubKey(@"S-1-5-20\Software\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform", true))
                {
                    if (registry != null)
                    {
                        registry.DeleteSubKeyTree("55c92734-d682-4d71-983e-d6ec3f16059f", false);
                    }
                }
            }
        }
        ...
        private static void RemoveKMSHostRegistry(LicenseList licenses, string registryPath)
        {
            using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(registryPath, true))
            {
                if (registry != null)
                {
                    ...
                    registry.DeleteValue("KeyManagementServiceName", false);
                    registry.DeleteValue("DisableDnsPublishing", false);
                    registry.DeleteValue("DisableKeyManagementServiceHostCaching", false);

                    // Only Search KMS Licenses
                    foreach (LicenseInstance license in licenses.GetListLicensedKMS())
                    {
                        ...
                        using (RegistryKey registrySubKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(registryPath + Path.DirectorySeparatorChar + license.AppID, true))
                        {
                            if (registrySubKey != null)
                            {
                                ...
                                registrySubKey.DeleteValue("KeyManagementServiceName", false);
                                ...
                                using (RegistryKey registrySubKey2 = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(registryPath + Path.DirectorySeparatorChar + license.AppID + Path.DirectorySeparatorChar + license.SKUID, true))
                                {
                                    if (registrySubKey2 != null)
                                    {
                                        ...
                                        registrySubKey2.DeleteValue("KeyManagementServiceName", false);
                                    }
                                }
                            }
                        }
            ...
            if (licenses is LicenseListWindows && OSVersion.GetWindowsNumber() >= 10 && licenses.GetListW10KMS().Count > 0)
            {
                foreach (LicenseInstance license in licenses.GetListW10KMS())
                {
                    using (RegistryKey kmsSubKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(registryPath + Path.DirectorySeparatorChar + license.AppID))
                    {
                        if (kmsSubKey != null)
                        {
                            using (RegistryKey kmsSubKey2 = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(registryPath + Path.DirectorySeparatorChar + license.AppID + Path.DirectorySeparatorChar + license.SKUID))
                            {
                                if (kmsSubKey2 != null)
                                {
                                    kmsSubKey2.SetValue("KeyManagementServiceName", "127.0.0.2");
                                    kmsSubKey2.SetValue("KeyManagementServicePort", "1688");
                                }
                            }
                        }
                    }
            ...
            {
                using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).OpenSubKey(registryPath, true))
                {
                    if (registry != null)
                    {
                        registry.DeleteValue("KeyManagementServiceName", false);
                        registry.DeleteValue("KeyManagementServicePort", false);
                        registry.DeleteSubKeyTree("0ff1ce15-a989-479d-af46-f275c6370663", false);
                    }
                }
            }
        ...
        */
        ...
        public static void SetKMSHostOffice(LicenseList licenses, string kmsServer)
        {
            ...
            else
            {
                ...
                SetKMSHostRegistry(licenses, @"SOFTWARE\Microsoft\OfficeSoftwareProtectionPlatform", kmsServer);
            }
        }
        ...
        {
            using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(registryPath))
            {
                if (registry != null)
                {
            ...
            if (licenses is LicenseListOffice && OfficeVersion.IsOfficeSPP())
            {
                using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(registryPath))
                {
                    if (registry != null)
                    {
                        registry.DeleteSubKeyTree("0ff1ce15-a989-479d-af46-f275c6370663", false);
                        using (RegistryKey kmsSubKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(registryPath + Path.DirectorySeparatorChar + "0ff1ce15-a989-479d-af46-f275c6370663"))
                        {
                            if (kmsSubKey != null)
                            {
                                kmsSubKey.SetValue("KeyManagementServiceName", kmsServer);
                            }
                        }
                    }
                }

                using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).CreateSubKey(registryPath))
                {
                    if (registry != null)
                    {
                        registry.SetValue("KeyManagementServiceName", kmsServer);
                        registry.DeleteSubKeyTree("0ff1ce15-a989-479d-af46-f275c6370663", false);
                        using (RegistryKey kmsSubKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).CreateSubKey(registryPath + Path.DirectorySeparatorChar + "0ff1ce15-a989-479d-af46-f275c6370663"))
                        {
                            if (kmsSubKey != null)
                            {
                                kmsSubKey.SetValue("KeyManagementServiceName", kmsServer);
        ...
        */
        ...
        public static void RemoveKMSPortOffice(LicenseList licenses)
        {
            ...
            else
            {
                ...
                RemoveKMSPortRegistry(licenses, @"SOFTWARE\Microsoft\OfficeSoftwareProtectionPlatform");
            }
        }
                    ...
                    foreach (LicenseInstance license in licenses.GetListLicensedKMS())
                    {
                        ...
                        using (RegistryKey registrySubKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(registryPath + Path.DirectorySeparatorChar + license.AppID, true))
                        {
                            if (registrySubKey != null)
                            {
                                ...
                                registrySubKey.DeleteValue("KeyManagementServicePort", false);
                                ...
                                using (RegistryKey registrySubKey2 = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(registryPath + Path.DirectorySeparatorChar + license.AppID + Path.DirectorySeparatorChar + license.SKUID, true))
                                {
                                    if (registrySubKey2 != null)
                                    {
                                        ...
                                        registrySubKey2.DeleteValue("KeyManagementServicePort", false);
                                    }
                                }
                            }
                        }
        ...
        */
        ...
        public static void SetKMSPortOffice(LicenseList licenses, string kmsPort)
        {
            ...
            else
            {
                ...
                SetKMSPortRegistry(licenses, @"SOFTWARE\Microsoft\OfficeSoftwareProtectionPlatform", kmsPort);
            }
        }
                ...
                using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).CreateSubKey(registryPath))
                {
                    if (registry != null)
                    {
                        ...
                        using (RegistryKey kmsSubKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).CreateSubKey(registryPath + Path.DirectorySeparatorChar + "0ff1ce15-a989-479d-af46-f275c6370663"))
                        {
                            if (kmsSubKey != null)
                            {
                                kmsSubKey.SetValue("KeyManagementServicePort", kmsPort);
                            }
                        }
                    }
                }
        ...
        */
        ...
        {
            string regKey = @"SOFTWARE\Policies\Microsoft\Windows NT\CurrentVersion\Software Protection Platform";
            using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(regKey))
            {
                if (registry != null)
                {
                    registry.SetValue("NoGenTicket", 1, RegistryValueKind.DWord);
                    registry.SetValue("NoAcquireGT", 1, RegistryValueKind.DWord);
                }
            }
        }
    }
    ...
    public static class KMSDLLInjection
    {
        ...
        public static void Initialize(string ip)
        {
            ...
            */
            ...
            {
                _targetDLL = Resources.SECOHook_64;
            }
            else if (Architecture.GetOSArch() == Architecture.A64)
            {
                _targetDLL = Resources.SECOHook_AA;
            }
            else
            {
                _targetDLL = Resources.SECOHook_32;
            }

            // Set DLL Path
            /*
            _pathDLL = Environment.GetEnvironmentVariable("windir") + @"\System32";
            if (File.Exists(Environment.GetEnvironmentVariable("windir") + @"\Sysnative\cmd.exe"))
            {
                _pathDLL = Environment.GetEnvironmentVariable("windir") + @"\Sysnative";
            }
            */
            if ((Architecture.GetOSArch() == Architecture.X64 || Architecture.GetOSArch() == Architecture.A64) && !Environment.Is64BitProcess)
            {
                _pathDLL = Environment.GetEnvironmentVariable("windir") + @"\Sysnative";
            }
            else
        ...
        public static bool UnloadDLL()
        {
            ...
            if (OSVersion.GetWindowsNumber() >= 10)
            {
        ...
        */
        ...
        public static void LoadDLLIFEO()
        {
            ...
            if (OSVersion.GetWindowsNumber() >= 6.3)
            {
                Services.WinDefendExclusion("Add", _pathDLL + "\\SECOHook.dll");
            }

            // Create DLL File
            CommonUtilities.FileCreate("SECOHook.dll", _targetDLL, _pathDLL);
            ...
            // Set Permissions on DLL File
            /*
            if (File.Exists(_pathDLL + "\\SECOHook.dll"))
            {
                // Get a FileSecurity object that represents the current security settings.
                FileSecurity fSecurity = File.GetAccessControl(_pathDLL + "\\SECOHook.dll");

                // Add the FileSystemAccessRule to the security settings.
                fSecurity.AddAccessRule(new FileSystemAccessRule(new SecurityIdentifier(WellKnownSidType.BuiltinUsersSid, null), FileSystemRights.FullControl, AccessControlType.Allow));

                // Set the new access settings.
                File.SetAccessControl(_pathDLL + "\\SECOHook.dll", fSecurity);
            }
            */

            // Create Image File Execution Options Settings
            string regKey = @"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\SppExtComObj.exe";
            using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(regKey))
            {
                if (registry != null)
                {
                    registry.SetValue("VerifierDlls", "SECOHook.dll", RegistryValueKind.String);
                    registry.SetValue("GlobalFlag", 256, RegistryValueKind.DWord);
                    registry.SetValue("VerifierDebug", 0, RegistryValueKind.DWord);
                    registry.SetValue("VerifierFlags", unchecked((int)2147483648), RegistryValueKind.DWord);
                    registry.DeleteValue("Debugger", false);
                }
            }
        }
        ...
        public static void UnloadDLLIFEO()
        {
            ...
            if (OSVersion.GetWindowsNumber() >= 6.3)
            {
                Services.StopService("SPPSVC");
            }
            CommonUtilities.KillProcess("SppExtComObj");
            Thread.Sleep(3000);

            // Delete Image File Execution Options Settings
            string regKey = @"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options";
            using (RegistryKey registry = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(regKey, true))
            {
                if (registry != null)
                {
                    registry.DeleteSubKey("SppExtComObj.exe", false);
                }
            }

            // Delete DLL File
            CommonUtilities.FileDelete(_pathDLL + "\\SECOHook.dll");
            /*
            if (File.Exists(Environment.GetEnvironmentVariable("windir") + @"\System32\SECOHook.dll"))
            {
                CommonUtilities.FileDelete(Environment.GetEnvironmentVariable("windir") + @"\System32\SECOHook.dll");
            }
            else if (File.Exists(Environment.GetEnvironmentVariable("windir") + @"\Sysnative\SECOHook.dll"))
            {
                CommonUtilities.FileDelete(Environment.GetEnvironmentVariable("windir") + @"\Sysnative\SECOHook.dll");
            }
            */
            ...
            if (OSVersion.GetWindowsNumber() >= 6.3)
            {
                Services.WinDefendExclusion("Remove", _pathDLL + "\\SECOHook.dll");
            }
        }
    ...
    public static class KMSTAPDriver
    {
        ...
        /// <summary>
        /// OS Version/Architecture Specific TAP drivers
        /// </summary>
        private static byte[] TAP_A;
        private static byte[] TAP_B;
        private static byte[] DevCon;
        ...
        private static void InstallTAPDeviceDriver()
        {
            ...
            if (OSVersion.GetWindowsNumber() >= 10 && OSVersion.GetWindowsBuildNumber() >= 14393)
            {
                if (Architecture.GetOSArch() == Architecture.X64)
                {
                    TAP_A = Resources.Otap10x64;
                    TAP_B = Resources.Vtap10x64;
                    DevCon = Resources.tapinstallx64;
                }
                else if (Architecture.GetOSArch() == Architecture.A64)
                {
                    TAP_A = Resources.Otap10a64;
                    TAP_B = Resources.Vtap10a64;
                    DevCon = Resources.tapinstalla64;
                }
                else
                {
                    TAP_A = Resources.Otap10x86;
                    TAP_B = Resources.Vtap10x86;
                    DevCon = Resources.tapinstallx86;
                }
            }
            else
            {
                if (Architecture.GetOSArch() == Architecture.X64)
                {
                    TAP_A = Resources.Otap6x64;
                    TAP_B = Resources.Vtap6x64;
                    DevCon = Resources.tapinstallx64;
                }
                else
                {
                    TAP_A = Resources.Otap6x86;
                    TAP_B = Resources.Vtap6x86;
                    DevCon = Resources.tapinstallx86;
                }
            }
            ...
            if (tapDrivers.Count > 0)
            {
                foreach (TAPDriver tapDriver in tapDrivers)
                {
                    if (tapDriver.Description.IndexOf("TAP-Windows Adapter V9", StringComparison.CurrentCultureIgnoreCase) >= 0)
                    {
                        ...
                        //CommonUtilities.FileCreate("tapinstall.exe", DevCon, installTAPAdapterFilePath);
                        using (IArchive aTAP = ArchiveFactory.Open(new MemoryStream(TAP_B)))
                        {
                            IReader reader = aTAP.ExtractAllEntries();
                            while (reader.MoveToNextEntry())
                            {
                                reader.WriteEntryToDirectory(CommonUtilities.EscapePath(installTAPAdapterFilePath), new ExtractionOptions() { ExtractFullPath = true, Overwrite = true });
                            }
                        }
                        using (IArchive aDEV = ArchiveFactory.Open(new MemoryStream(DevCon)))
                        {
                            IReader reader = aDEV.ExtractAllEntries();
                            while (reader.MoveToNextEntry())
                            {
                                reader.WriteEntryToDirectory(CommonUtilities.EscapePath(installTAPAdapterFilePath), new ExtractionOptions() { ExtractFullPath = true, Overwrite = true });
                            }
                    ...
                    if (tapDriver.Description.IndexOf("Viscosity Virtual Adapter V9.1", StringComparison.CurrentCultureIgnoreCase) >= 0)
                    {
                        ...
                        //CommonUtilities.FileCreate("tapinstall.exe", DevCon, installTAPAdapterFilePath);
                        using (IArchive aTAP = ArchiveFactory.Open(new MemoryStream(TAP_A)))
                        {
                            IReader reader = aTAP.ExtractAllEntries();
                            while (reader.MoveToNextEntry())
                            {
    ...
    public static class KMSWinDivert
    {
        ...
        public static void StartWinDivertClient(string ip, string subnet)
        {
            ...
            if (OSVersion.GetWindowsNumber() >= 6.3)
            {
                Services.WinDefendExclusion("Add", CommonUtilities.EscapePath(winDivertLoaderFilePath));
            }
        ...
        public static void StopWinDivertClient()
        {
            ...
            try
            {
                ...
                if (OSVersion.GetWindowsNumber() >= 6.3)
                {
                    Services.WinDefendExclusion("Remove", CommonUtilities.EscapePath(winDivertLoaderFilePath));
                }
