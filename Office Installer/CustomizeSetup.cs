...
namespace OfficeInstaller
{
    ...
    public static class CustomizeSetup
    {
        ...
        public static void AddUpdate(string filePath)
        {
            ...
            string[] xmlList = Directory.GetFiles(updateFolderPath, "*.xml");
            if (xmlList.Length > 0)
            {
                foreach (string xmlName in xmlList)
                {
                    //CommonUtilities.FileDelete(updateFolderPath + Path.DirectorySeparatorChar + Path.GetFileName(xmlName));
                    CommonUtilities.FileDelete(xmlName);
                }
            }

            if (InstallerInformation.SetupVersionMajor == 14)
            {
                string nameFile = Path.GetFileName(filePath).ToLower();

                if ( nameFile.Contains("kb2687455") || nameFile.Contains("kb2687449") || nameFile.Contains("kb2687457") || nameFile.Contains("kb2687468") || nameFile.Contains("kb2687463") )
                {
                    string[] sp2shared = { "clientshared32ww-x-none", "clientshared64ww-x-none", "officesuiteww-x-none", "projectww-x-none", "visioww-x-none", "sharepointdesignerww-x-none", "imeww-x-none", "proofkitww-x-none" };
                    string[] sp2short = { "wwlibcxm-en-us", "accessde-de-de", "accessde-en-us", "accessde-es-es", "accessde-fr-fr", "accessde-it-it", "accessde-ja-jp", "accessde-ko-kr", "accessde-zh-cn", "accessde-zh-tw", "ime32-ja-jp", "ime64-ja-jp", "ime32-ko-kr", "ime64-ko-kr", "ime32-zh-cn", "ime64-zh-cn", "ime32-zh-tw", "ime64-zh-tw" };
                    List<string> sp2all = new List<string>();
                    sp2all.AddRange(sp2shared);
                    sp2all.AddRange(sp2short);

                    foreach (string llcc in new LanguageList().GetLanguageCodes())
                    {
                        string[] sp2long;
                        sp2long = new string[] { "clientshared32mui-" + llcc, "clientshared64mui-" + llcc, "clientsharedmui-" + llcc, "officesuitemui-" + llcc, "osamui-" + llcc, "proofing-" + llcc, "proof-" + llcc, "projectmui-" + llcc, "visiomui-" + llcc, "sharepointdesignermui-" + llcc };
                        sp2all.AddRange(sp2long);
                    }

                    string[] sp2files = sp2all.ToArray();

                    foreach (string mspFile in sp2files)
                    {
                        string sourceFile = mspFile + ".msp";
                        string targetFile = sourceFile.Insert(0, "pre_");
                        if (File.Exists(updateFolderPath + "\\" + sourceFile))
                        {
                            if (File.Exists(updateFolderPath + "\\" + targetFile))
                            {
                                CommonUtilities.FileDelete(updateFolderPath + "\\" + sourceFile);
                            }
                            else
                            {
                                File.Move(updateFolderPath + "\\" + sourceFile, updateFolderPath + "\\" + targetFile);
                            }
                        }
                    }

                    if (File.Exists(updateFolderPath + "\\eula.txt"))
                    {
                        CommonUtilities.FileDelete(updateFolderPath + "\\eula.txt");
                    }
                }

                if (!File.Exists(updateFolderPath + "\\ose.exe"))
                {
                    byte[] Updates32 = Resources.Office14_Updates32;
                    byte[] Updates64 = Resources.Office14_Updates64;

                    if (InstallerInformation.InstallerArchitecture == Architecture.X86)
                    {
                        using (IArchive aUpdates = ArchiveFactory.Open(new MemoryStream(Updates32)))
                        {
                            IReader reader = aUpdates.ExtractAllEntries();
                            while (reader.MoveToNextEntry())
                            {
                                if (!reader.Entry.IsDirectory)
                                {
                                    reader.WriteEntryToDirectory(updateFolderPath, new ExtractionOptions() { ExtractFullPath = true, Overwrite = true });
                                }
                            }
                        }
                    }
                    ...
                    {
                        using (IArchive aUpdates = ArchiveFactory.Open(new MemoryStream(Updates64)))
                        {
                            IReader reader = aUpdates.ExtractAllEntries();
                            while (reader.MoveToNextEntry())
                            {
        ...
        private static byte[] DecompressResource(byte[] compressedResource)
        {
            // Decompress XML
        ...
        */
        ...
        public static void ChangeChannel(string licenseChannel, string productName)
        {
            ...
            */
            ...
            if (patchXml != null)
            {
                doc.Load(new MemoryStream(DecompressResource(patchXml)));
            }
    ...
    public class LanguageList
    {
        ...
        static LanguageList()
        {
            ...
            // Office 2016 Languages
            AddLanguage("id-id", "Indonesian");
            AddLanguage("kk-kz", "Kazakh");
            AddLanguage("ms-my", "Malay");
            AddLanguage("vi-vn", "Vietnamese");
        }
